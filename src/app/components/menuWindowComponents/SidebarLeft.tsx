...
import { comandasContext } from "../../context/ComandaContexts";//"@/app/context/ComandaContexts";

interface SidebarLeftProps {
  products: any; // Cambiar`any` a un tipo de los productos
}

// let newProducts: any[] = [];

export const SidebarLeft = () => {
  const { products } = useContext(comandasContext);
  const [newProducts, setNewProducts] = useState<any[]>([]);
  const mostrar = () => {
    console.table(products)
    newProducts.map(product => console.log(product));
  }
  ...
  const deleteProduct = (delProduct: number) => {
    setNewProducts(prevProducts => prevProducts.filter(product => product !== delProduct))
  };
